	...
	internal class JSONSerializer
		...
		private void WriteObject(object obj)
			...
			if (useExtension)
				{
#if gtrm
				// adding the assembly name severely limits object sharing & portabilty
				//the namespace should be used to prevent collisions...
		...
		private void Indent(char c)
			...
			if (_Indent)
				...
				for (int i = 0; i < _current_depth - 1; i++)
					_output.Append("   ");
				if (c != 0)
					_output.Append(c);
				...
				if (c == '}')
					...
					_current_depth--;
					if (_current_depth < 0)
						throw new Exception("Serializer encountered depth underflow!");
					}
				}
			}
